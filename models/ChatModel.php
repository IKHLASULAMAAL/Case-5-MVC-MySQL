...
class ChatModel
{
    ...
    {
        $query = "SELECT * FROM pesan WHERE (pengirim = ? AND penerima = ?) OR (pengirim = ? AND penerima = ?) ORDER BY waktu ASC";
        $stmt = $this->pdo->prepare($query);
    ...
    {
        $query = "INSERT INTO pesan (pengirim, penerima, pesan, waktu) VALUES (?, ?, ?, NOW())";
        $stmt = $this->pdo->prepare($query);
